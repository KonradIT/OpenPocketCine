...
public enum LiveFrameTap {
    ...
    private static func readBiPlanar(
        ...
        let lumaScale: Double =
            tenBit ? (videoRange ? 255.0 / 876.0 : 255.0 / 1023.0) : (videoRange ? 255.0 / 219.0 : 1)
        let chromaCenter: Double = tenBit ? 512 : 128
        let chromaScale: Double =
            tenBit ? (videoRange ? 255.0 / 896.0 : 255.0 / 1023.0) : (videoRange ? 255.0 / 224.0 : 1)
        ...
        for y in 0..<height {
            ...
            for x in 0..<width {
                ...
                if tenBit {
                    ...
                    luma = Double(lumaBase.load(fromByteOffset: sy * lumaBPR + sx, as: UInt8.self))
                    cb = Double(chromaBase.load(fromByteOffset: cy * chromaBPR + cx * 2, as: UInt8.self))
                    cr = Double(chromaBase.load(fromByteOffset: cy * chromaBPR + cx * 2 + 1, as: UInt8.self))
                }
    ...
    private static func ciFallback(_ buffer: CVPixelBuffer, maxWidth: Int) -> (
        ...
        guard CVPixelBufferLockBaseAddress(cpu, .readOnly) == kCVReturnSuccess else { return nil }
        defer { CVPixelBufferUnlockBaseAddress(cpu, .readOnly) }
        var bytes = [UInt8](repeating: 0, count: width * height * 4)
        let ok = readPacked32(cpu, into: &bytes, width: width, height: height, step: 1, bgra: true)
        return ok ? (bytes, width, height, width * 4) : nil
    }
    #else
    private static func ciFallback(_ buffer: CVPixelBuffer, maxWidth: Int) -> (
