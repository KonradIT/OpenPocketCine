...
/// boundary.
///
/// Copied from OpenZCine `Sources/OpenZCineCore/LinkSignalBars.swift`. Raw bands are health
/// quartiles (matching the connection panel's meter): 1–25 → 1 bar, 26–50 → 2, 51–75 → 3,
/// 76–100 → 4; score 0 → 0 bars. Changing bands requires the score to clear the target band's
/// floor by the hysteresis margin (going up) or drop that margin below the current band's floor
/// (going down). Zero is immediate in both directions — losing or gaining the link should never
/// be damped.
///
