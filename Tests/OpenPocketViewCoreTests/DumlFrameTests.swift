...
@Suite struct DumlFrameTests {
    ...
    @Test func encodeEnterPlayback() {
        let frame = Duml.encode(.init(sender: Duml.senderApp, receiver: Duml.rxCamera, seq: 0xA000,
                                      flags: Duml.flagRequest, cmdSet: 0x02, cmdId: 0x0C,
                                      payload: [0x01, 0x01, 0x00, 0x01]))
        #expect(frame == [0x55, 0x11, 0x04, 0x92, 0x02, 0x01, 0x00, 0xA0,
    ...
    @Test func encodePairing() {
        ...
        let frame = Duml.encode(.init(sender: Duml.senderApp, receiver: Duml.rxWifi, seq: 0x8092,
                                      flags: Duml.flagRequest, cmdSet: 0x07, cmdId: 0x45, payload: pin))
        #expect(frame.count == 51)
        #expect(frame[3] == 0xC2)
        #expect(Array(frame.suffix(2)) == [0xA0, 0xB4])
        #expect(Array(frame.prefix(11)) == [0x55, 0x33, 0x04, 0xC2, 0x02, 0x07, 0x92, 0x80, 0x40, 0x07, 0x45])
    }

    @Test func roundTrip() throws {
        let original = Duml.Frame(sender: Duml.senderApp, receiver: Duml.rxCamera, seq: 0xA000,
    ...
    @Test func decodeRejectsBadCrc() {
        var bytes = Duml.encode(.init(sender: Duml.senderApp, receiver: Duml.rxCamera, seq: 0xA000,
