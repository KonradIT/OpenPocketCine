...
@Suite struct BleAdvertTests {
    ...
    @Test func pocket4ProNewFormat() {
        let payload: [UInt8] = [0x00, 0x00, 0x00, 0xEE, 0x00, 0x04, 0xBD, 0x6E, 0x56, 0x20, 0xDA, 0x00, 0x00, 0x10]
    ...
    @Test func modelResolvesToPocket() {
        ...
        #expect(CameraModel.resolve(modelId: nil, name: "OsmoPocket4P-AAAA").name == "Osmo Pocket 4 Pro")
...
@Suite struct CommandsTests {
    @Test func pairingFrameMatchesKnownBytes() {
        ...
        #expect(f.count == 51)
        #expect(Array(f.prefix(11)) == [0x55, 0x33, 0x04, 0xC2, 0x02, 0x07, 0x92, 0x80, 0x40, 0x07, 0x45])
    }
    ...
    @Test func cameraControlFrames() {
        ...
        #expect(Commands.setShutter(denom: 40).payload == [0x01, 0x28, 0x80, 0x00, 0x00, 0x00, 0x40])
        ...
        manualExpo[0] = 0x30; manualExpo[1] = 0x02; manualExpo[7] = 0x04
...
@Suite struct CameraStatusTests {
    ...
    @Test func batteryDecode() {
        ...
        p[1] = 0x10; p[2] = 0x0F          // mV = 0x0F10 = 3856
        p[20] = 85                        // percent
        p[27] = 1                         // docked
        p[32] = 1                         // charging
        var s = CameraStatus()
        let handled = CameraStatusDecoder.apply(
            .init(sender: 0, receiver: 0, seq: 0, flags: 0, cmdSet: 0x0D, cmdId: 0x02, payload: p), to: &s)
        #expect(handled)
    ...
    @Test func storageDecodeTwoStores() {
        var p = [UInt8](repeating: 0, count: 40)
        func put(_ v: UInt32, _ i: Int) { p[i] = UInt8(v & 0xFF); p[i+1] = UInt8((v >> 8) & 0xFF) }
        put(64000, 6); put(32000, 10)     // SD total/free
        put(48980, 24); put(1000, 28)     // built-in total/free
        var s = CameraStatus()
        #expect(CameraStatusDecoder.apply(
            .init(sender: 0, receiver: 0, seq: 0, flags: 0, cmdSet: 0x02, cmdId: 0xDC, payload: p), to: &s))
    ...
    @Test func gimbalHeartbeatSwallowed() {
        var s = CameraStatus()
        #expect(CameraStatusDecoder.apply(
            .init(sender: 0, receiver: 0, seq: 0, flags: 0, cmdSet: 0x04, cmdId: 0x05, payload: []), to: &s))
    }
    ...
    @Test func stateInfoRecordingAndMode() {
        ...
        p[0] = 0x01
        p[5] = 0x10; p[9] = 0x08          // storage total/free (plausible MiB)
        p[57] = 0x01                      // Video
        var s = CameraStatus()
        #expect(CameraStatusDecoder.apply(
            .init(sender: 0, receiver: 0, seq: 0, flags: 0, cmdSet: 0x02, cmdId: 0x80, payload: p), to: &s))
    ...
    @Test func timecodePushFromMimo() {
        ...
        var s = CameraStatus()
        #expect(CameraStatusDecoder.apply(
            .init(sender: 0, receiver: 0, seq: 0, flags: 0, cmdSet: 0x00, cmdId: 0x99, payload: payload), to: &s))
    ...
    @Test func expoAndFpsPushesFromLive1() {
        ...
        expo[2] = 0x40; expo[3] = 0x86     // 1/1600 = 0x8640 (denom | 0x8000)
        expo[5] = 0x04                     // ISO index 200
        expo[13] = 0xC8; expo[14] = 0x00   // leftover 200 — not ISO
        expo[16] = 0xC8; expo[17] = 0x00   // ISO value 200
        var s = CameraStatus()
    ...
    @Test func subscribeAckIsIgnored() {
        var s = CameraStatus()
        #expect(!CameraStatusDecoder.apply(
            .init(sender: 0, receiver: 0, seq: 0, flags: 0xC0, cmdSet: 0x00, cmdId: 0x99, payload: [0x00]), to: &s))
    }
}
