...
@Suite struct DumlTransportTests {
    ...
    @Test func transportHeaderWorkedExample() {
        let hdr = DumlTransport.transportHeader(pktType: 0x05, payloadLen: 29, sessionId: 0x3A7C, seq: 0xB890)
    ...
    @Test func transportSeqReadsBytes4And5() {
        let hdr = DumlTransport.transportHeader(pktType: 0x02, payloadLen: 100, sessionId: 0x0992, seq: 0xA8B0)
    ...
    @Test func transportHeaderXorIsSelfChecking() {
        let hdr = DumlTransport.transportHeader(pktType: 0x00, payloadLen: 40, sessionId: 0x1234, seq: 0x5678)
        #expect(hdr.count == 8)
    ...
    @Test func handshakeDatagramIsPktType00() {
        let hdr = DumlTransport.transportHeader(pktType: 0x00, payloadLen: 40, sessionId: 0x1234, seq: 0)
        #expect(DumlTransport.isHandshake(hdr))
        #expect(!DumlTransport.isHandshake(DumlTransport.transportHeader(
    ...
    @Test func scanFindsWrappedFrame() {
        let packet: [UInt8] = [
            ...
            0x55, 0x11, 0x04, 0x92, 0x02, 0x01, 0x00, 0xA0, 0x40, 0x02, 0x0C, 0x01, 0x01, 0x00, 0x01, 0xB6, 0x3B,  // DUML
        ]
