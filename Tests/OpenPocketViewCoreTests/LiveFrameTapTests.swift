...
@Suite struct LiveFrameTapTests {
        ...
        for y in 0..<height {
            for x in 0..<width {
                ...
                if tenBit {
                    ...
                    lumaBase.storeBytes(
                        of: UInt16(value << 6), toByteOffset: y * lumaBPR + x * 2, as: UInt16.self)
                } else {
                    lumaBase.storeBytes(
            ...
            for x in 0..<(width / 2) {
                if tenBit {
                    chromaBase.storeBytes(
                        of: UInt16(cb << 6), toByteOffset: y * chromaBPR + x * 4, as: UInt16.self)
                    chromaBase.storeBytes(
                        of: UInt16(cr << 6), toByteOffset: y * chromaBPR + x * 4 + 2, as: UInt16.self)
                } else {
                    chromaBase.storeBytes(
    ...
    private func lumaBytes(_ tap: (bytes: [UInt8], width: Int, height: Int, bytesPerRow: Int)) -> [UInt8] {
